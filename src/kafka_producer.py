...
class KafkaLoans:
    ...
    def load(self, df):
        if not os.path.exists("data_csv"):
            df.repartition('region').write.mode('overwrite').options(header="true", inferSchema="true", delimiter=",").csv("file:///"+str(Path(__file__).parent)+"/data_csv")
    ...
    def kafka_stream_in(self):
        ...
        for file in files:
            with open("data_csv/"+file, mode='r') as csv_file:
