class BankLoan:
    ...
    def transform(self, df):
        ...
        bsq1 = df.withColumn(
            ...
            F.when(
                F.col("loan_status") == "Fully Repaid",
        ...
        set_ = set(
            df.select("country")
            .filter(F.col("loan_status").isin("Fully Cancelled", "Cancelled"))
        ...
        bsq4 = (
            df.groupBy("country")
            .agg(
                F.round(F.avg("time_taken_for_repayment")).alias(
                    "Average_repayment_days"
                )
            )
        ...
        bsq6 = (
            df.groupBy("project_name", "country")
            .agg(F.count("*").alias("Number of Occurance"))
        ...
        total_amount = int(
            list(
                df.select(F.sum("orig_prin_amount")).rdd.flatMap(lambda x: x).collect()
            )[0]
        )
        bsq7 = (
            df.groupBy("region")
            ...
            .withColumn(
                "Percentage of Loan",
                F.round((F.col("Original Principle Amount") * 100) / total_amount, 0),
            )
            .orderBy(F.col("Percentage of Loan").desc())
        )

        # Amount of Loan each year for every region
        bsq8 = df.groupBy("region", F.year("end_of_period")).agg(
            F.sum("orig_prin_amount").alias("Sum_Principle_Amount")
        )

        # Region with the highest cancellation amount segregated with loan type
        bsq9 = df.groupBy("region", "loan_type").agg(
            F.sum("cancelled_amount").alias("cancelled_amount")
        )
