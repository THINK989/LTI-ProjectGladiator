...
def handling_null(df):
    ...
    for key, val in type_dict.items():
        ...
        # * If data is of type string
            ...
            with open("country.csv", mode="r") as country_code:
                ...
                for country_code, country in code_reader:
                    ...
                    df = df.withColumn(
                        "gaurantor_country_clone",
                        F.when(
                            F.col("gaurantor_countrycode") == country_code, country
                        ).otherwise(F.col("gaurantor_country")),
                    ).withColumn(
                        "gaurantor_countrycode_clone",
                        F.when(
                            F.col("gaurantor_country") == country, country_code
                        ).otherwise(F.col("gaurantor_countrycode")),
                    )
        ...
        # * If data is of type date
            ...
            df = (
                df.withColumn(
                    "days_to_sign_the_loan",
                    F.when(
                        F.col("agreement_signing_date").isNull()
                        | F.col("board_approval_date").isNull(),
                        F.lit(float("inf")),
                    ).otherwise(
                        F.datediff("agreement_signing_date", "board_approval_date")
                    ),
                )
                .withColumn(
                    "time_taken_for_repayment",
                    F.when(
                        F.col("last_repayment_date").isNull()
                        | F.col("first_repayment_date").isNull(),
                        F.lit(float("inf")),
                    ).otherwise(
                        F.datediff("last_repayment_date", "first_repayment_date")
                    ),
                )
...
def string_handling(df):
    ...
    return (
        df.withColumn("region_upper", F.upper(F.col("region")))
        .drop("region")
...
def translator_dict(df):
    with open("borrower.csv", "r") as csv_file:
        ...
        next(reader, None)
        borrower_translation = dict(reader)
    translator_mapping = F.create_map(
        [F.lit(x) for x in chain(*borrower_translation.items())]
    )

    df = df.withColumn(
        "borrower_clone",
        F.coalesce(translator_mapping[F.col("borrower")], F.col("borrower")),
    )
...
def clean_borrower(df):
    ...
    df = (
        df.withColumn(
            "borrower_upper",
            F.regexp_replace(F.upper(F.col("borrower")), "[\n\r]|[^A-Z ,&.]", ""),
        )
...
def preprocess(df):
    return string_handling(
        clean_borrower(
            translator_dict(handling_null(country_code_hashmap(delete_columns(df))))
        )
    )
